...
func SendMessageV2(c *gin.Context) {
	...
	//redis.Set(context.Background(), "sendmessage:"+c.ClientIP(), content, 2*time.Second)
	//val, err := redis.Get(context.Background(), "sendmessage:"+c.ClientIP()).Result()
	//if err != nil {
	//	tools.Logger().Print("redis get err:", err)
	//}
	//tools.Logger().Print("redis:", val)
